...
@section('title', "{$unit->catalog->name} on {$unit->equipment->name}")

@section('content')
    <form action="{{ route('admin.units.update', $unit) }}" method="post" autocomplete="off" enctype="multipart/form-data">
        @method('PUT')
        @csrf
