                            ...
                            <div class="drawing__figure-inner">
                                ...
                                @foreach($nodes as $node)
                                    @if($node->pos_x != 0 && $node->pos_y != 0)
                                        <div class="item-block" data-target="targetConstructorNode_{{ $node->nid }}" style="position: absolute; top: {{ $node->pos_y - ($node->size_y / 2) }}px; left: {{ $node->pos_x - ($node->size_x / 2) }}px; border: 1px solid {{ $node->color }}; z-index: 9999; width: {{ $node->size_x }}px; height: {{ $node->size_y }}px;"></div>
                                    @endif
                                @endforeach
                            </div>
